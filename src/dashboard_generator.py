  ...
  <script>
    ...
    function renderDealRoom(idx) {{
      ...
      const html = `
        ...
        <div class="deal-room-header p-3 p-md-4 rounded-top-3 border-bottom border-secondary border-opacity-25" style="background: linear-gradient(135deg, #090e17 0%, #0f172a 100%);">
          <!-- Mobile Back Button -->
          ...
          <!-- Top Row: Category & Intelligence Badges -->
          <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-2 pb-2 border-bottom border-white border-opacity-10">
            <div class="d-flex flex-wrap align-items-center gap-2">
              <span class="badge ${{lead.badge_cls}} py-1 px-2 fw-bold">${{lead.badge_text}}</span>
              <span class="badge bg-danger text-white py-1 px-2 fw-semibold">
                <i class="fa-solid fa-bullseye me-1"></i> Conversion Score: ${{lead.score_val}}%
              </span>
              <span class="badge bg-warning-subtle text-dark py-1 px-2 fw-semibold">
                <i class="fa-solid fa-shield-check text-success me-1"></i> Verified Importer
              </span>
            </div>
            <div class="d-flex align-items-center">
              <span class="badge ${{deal.pulse_badge || 'bg-danger'}} py-1 px-3 fw-bold cursor-pointer shadow-sm" onclick="showTimingCalculation(event, ${{idx}})" title="Click to view Month 0 timing breakdown">
                ${{deal.status_tag || '⏰ Month 0 Window'}} <i class="fa-solid fa-circle-question ms-1"></i>
              </span>
            </div>
          </div>

          <!-- Main Row: Company Name & Action Toolbar -->
          <div class="d-flex flex-column flex-lg-row justify-content-between align-items-start align-items-lg-center gap-3 my-2">
            <div>
              <h2 class="h4 fw-bold text-white mb-1 text-break" style="letter-spacing: -0.3px;">
                ${{lead.company}}
              </h2>
              <div class="text-slate-300 small opacity-90">
                <i class="fa-solid fa-file-invoice text-warning me-1"></i> ${{lead.institution}} • <span class="font-monospace text-slate-200">${{lead.tender_ref}}</span>
              </div>
            </div>

            <!-- Action Toolbar (Equal height, high contrast, sleek pill buttons) -->
            <div class="d-flex flex-wrap align-items-center gap-2 flex-shrink-0">
              <button class="btn btn-success fw-bold px-3 py-2 shadow-sm d-inline-flex align-items-center gap-2" style="background-color: #10b981; border-color: #10b981; font-size: 0.85rem;" onclick="openPitchModal('whatsapp')">
                <i class="fa-solid fa-paper-plane"></i> Contact & Pitch Hub
              </button>
              <button class="btn btn-outline-light fw-semibold px-3 py-2 d-inline-flex align-items-center gap-2" style="font-size: 0.85rem;" onclick="exportExecutivePDF()">
                <i class="fa-solid fa-file-pdf text-danger"></i> PDF
              </button>
              <button class="btn btn-outline-info fw-semibold px-3 py-2 d-inline-flex align-items-center gap-2" style="font-size: 0.85rem;" data-bs-toggle="modal" data-bs-target="#${{lead.hs_modal_id}}">
                <i class="fa-solid fa-barcode"></i> HS Codes
              </button>
            </div>
          </div>

          <!-- Bottom Row: Executive Contact Quick Strip -->
          <div class="d-flex flex-wrap align-items-center gap-3 gap-md-4 pt-2 mt-2 border-top border-white border-opacity-10 small text-slate-300">
            <div>
              <i class="fa-brands fa-whatsapp text-success me-1"></i> Direct Mobile: <strong class="text-white font-monospace">${{directPhone}}</strong>
            </div>
            <div>
              <i class="fa-solid fa-user-tie text-info me-1"></i> Key Contact: <strong class="text-white">${{procName}}</strong> <span class="opacity-75">(${{procTitle}})</span>
            </div>
            <div>
              <i class="fa-solid fa-location-dot text-danger me-1"></i> Location: <span class="text-slate-200">${{address.split(',')[0]}}, ${{lead.country || 'Malawi'}}</span>
            </div>
          </div>
        </div>
        ...
        <div class="p-3 p-md-4 bg-light">
          ...
          <div class="row g-2 g-md-3 mb-3 mb-md-4">
            <div class="col-6 col-md-3">
              <div class="kpi-card text-center p-2 p-md-3 rounded-3 bg-white shadow-sm border">
                <div class="kpi-val text-primary">${{lead.turnover_num || '$680.00K'}}</div>
                <div class="kpi-lbl">IMPORT TURNOVER</div>
              </div>
            </div>
            <div class="col-6 col-md-3">
              <div class="kpi-card text-center p-2 p-md-3 rounded-3 bg-white shadow-sm border">
                <div class="kpi-val text-dark">${{lead.shipments_num || '72'}}</div>
                <div class="kpi-lbl">IMPORT SHIPMENTS</div>
              </div>
            </div>
            <div class="col-6 col-md-3">
              <div class="kpi-card text-center p-2 p-md-3 rounded-3 bg-white shadow-sm border">
                <div class="kpi-val text-warning text-truncate">${{lead.primary_port || (lead.entry_ports ? lead.entry_ports.split(',')[0].trim() : 'Songwe Border')}}</div>
                <div class="kpi-lbl">PRIMARY ENTRY PORT</div>
              </div>
            </div>
            <div class="col-6 col-md-3">
              <div class="kpi-card text-center p-2 p-md-3 rounded-3 bg-white shadow-sm border">
                <div class="kpi-val text-success text-truncate">${{deal.sourcing_hubs || (lead.sourcing_countries ? lead.sourcing_countries.split(',')[0].trim() : 'China & UAE')}}</div>
          ...
          <div class="card p-3 p-md-4 border-0 rounded-3 bg-white shadow-sm mb-3 mb-md-4 border-top border-success border-3">
